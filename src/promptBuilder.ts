/**
 * Prompt Builder - generates tool usage instructions for system prompt injection
 * Follows OpenAI API documentation tool definitions exactly
 */
...
function getToolParameters(tool: Record<string, unknown>): ParamEntry[] {
  ...
  // custom / namespace / unknown: try to extract from parameters if present
...
/**
 * Generate a complete tool usage prompt from an array of OpenAI tool definitions.
 * Follows the XML-style format specified by the user.
 */
