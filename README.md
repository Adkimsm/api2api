...
- 可在管理后台修改 `ADMIN_TOKEN` 和 `SERVICE_API_KEY`
...
9. 如需轮换 `ADMIN_TOKEN` 或 `SERVICE_API_KEY`，在「概览」的安全配置中保存新值。
...
部署后可以在管理后台修改 `ADMIN_TOKEN` 和 `SERVICE_API_KEY`。后台保存的值存入 D1 并优先于 Cloudflare Secret；如果 D1 中没有覆盖值，则继续使用 Secret 作为兜底。`API_KEY_ENCRYPTION_SECRET` 不允许从后台修改，避免已有 provider API Key 无法解密。
...
- `SERVICE_API_KEY`：保存后立即作为 OpenAI-compatible API 的 Bearer Token 生效。
- `ADMIN_TOKEN`：后端不会返回明文；修改时需要输入当前 `ADMIN_TOKEN` 和新的 `ADMIN_TOKEN`，保存成功后需要重新登录。

这些覆盖配置保存在 D1 的 `app_settings` 表中。Cloudflare Secret 仍作为初始值和兜底值使用。

`API_KEY_ENCRYPTION_SECRET` 只从 Cloudflare Secret 读取，不支持后台修改。
